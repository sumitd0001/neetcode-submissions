class Solution {
    ...
    bool hasCycle(vector<vector<int>>& adj, vector<int>& state, vector<int>& result, int curr) {
        ...
        for (const auto& next : adj[curr]) {
            ...
            if (hasCycle(adj, state, result, next)) return true;
        }
    ...
    vector<int> findOrder(int numCourses, vector<vector<int>>& prerequisites) {
        ...
        for (int i = 0; i < numCourses; ++i) {
            if (state[i]==0) {
                if (hasCycle(adj, state, result, i)) return {};
