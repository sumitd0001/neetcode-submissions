...
public:
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
       ...
       for (int i = 0; i<r; ++i) {
            if (!pacific_visted[i][0]) bfs(heights, pacific_visted, r, c, i, 0); // left , pacific
            if (!atlantic_visted[i][c-1]) bfs(heights, atlantic_visted, r, c, i, c-1); // right , atlantic
        }

        for (int j = 0; j<c; ++j) {
            if (!pacific_visted[0][j]) bfs(heights, pacific_visted, r, c, 0, j); // top , pacific
            if (!atlantic_visted[r-1][j]) bfs(heights, atlantic_visted, r, c, r-1, j); // bottom , atlantic
        }
